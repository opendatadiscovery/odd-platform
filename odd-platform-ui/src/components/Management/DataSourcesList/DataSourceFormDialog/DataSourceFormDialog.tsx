...
interface DataSourceFormDialogProps extends StylesType {
  ...
  searchNamespace: (params: NamespaceApiGetNamespaceListRequest) => Promise<NamespaceList>;
}
...
const DataSourceFormDialog: React.FC<DataSourceFormDialogProps> = ({
  ...
  type FilterOption = Omit<Namespace, 'id' | 'namespace'> & Partial<Namespace>;
  ...
  const handleNamespaceSearch = React.useCallback(
    useDebouncedCallback(() => {
      setLoading(true);
      searchNamespace({ query: searchText, page: 1, size: 30 }).then(response => {
