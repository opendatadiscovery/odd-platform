...
public class CollectorServiceImpl implements CollectorService {
    ...
    public Mono<CollectorList> list(final int page, final int size, final String nameQuery) {
        return collectorRepository.listDto(page, size, nameQuery).map(collectorMapper::mapPojos);
    }
    ...
    public Mono<Collector> update(final long id, final CollectorUpdateFormData form) {
        if (StringUtils.isNotEmpty(form.getNamespaceName())) {
            ...
            return Mono.zip(collectorRepository.getDto(id), namespace)
                .flatMap(t -> update(
                    collectorMapper.applyForm(t.getT1().collectorPojo(), form),
                    t.getT2(),
                    t.getT1().tokenDto())
                );
        }
        ...
        return collectorRepository.getDto(id)
            .flatMap(c -> update(collectorMapper.applyForm(c.collectorPojo(), form), null, c.tokenDto()));
    }
...
//            }));
    ...
    private Mono<Collector> create(final CollectorFormData form,
        ...
        final CollectorPojo toCreate = collectorMapper.mapFormToPojo(form)
