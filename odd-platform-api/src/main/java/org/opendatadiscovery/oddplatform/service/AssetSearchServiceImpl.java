...
public class AssetSearchServiceImpl implements AssetSearchService {
    ...
    public Mono<AssetList> searchAssets(final AssetSearchFormData formData, final Integer size,
        ...
        if (state.isMyObjects()) {
            ...
            return authIdentityProvider.fetchAssociatedOwner()
                .flatMap(owner -> resolvePage(state, assetKinds, owner, sort, cursor, cappedSize))
    ...
    private Mono<AssetList> resolvePage(final FacetStateDto state, final List<String> assetKinds,
                                        final OwnerPojo owner, final SearchSortDto sort,
        ...
        if (relevance && relevanceOffset >= RELEVANCE_MAX_DEPTH) {
            ...
            return assetSearchRepository.count(state, assetKinds, owner)
        ...
        final var pageFlux = relevance
            ? assetSearchRepository.relevancePage(state, assetKinds, owner, relevanceOffset, fetchLimit)
            : assetSearchRepository.keysetPage(state, assetKinds, owner, cursor, fetchLimit);

        return Mono.zip(pageFlux.collectList(), assetSearchRepository.count(state, assetKinds, owner))
