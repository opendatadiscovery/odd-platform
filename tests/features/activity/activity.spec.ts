...
test.describe('Check filters', () => {
    test.describe('When apply single filter', () => {
        ...
        for (const parameter of parameters) {
            test(`Display an expected item with filter ${parameter.testName}`, async ({
                ...
                await test.step(`When apply filter ${parameter.testName}`, async () => {
                    ...
                    expect(await pages.activity.countListItems(entityNameWithAlphabeticChars)).toBeGreaterThanOrEqual(1);
                });
            });
        };
        /**
         ...
         */
            test(`Display an expected item with filter Tag`, async ({
                                                                                          steps: {pages},
                                                                                      }) => {
         ...
         */
        test(`Display an expected item with filter Owner`, async ({workerId,
                                                                    steps: {pages},
                                                                }) => {
         ...
         */
        test(`Display an expected item with filter Period`, async ({
                                                                           steps: {pages},
                                                                       }) => {
